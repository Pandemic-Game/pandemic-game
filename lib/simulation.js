...
class PandemicEnv {
  ...
  step(action_r) {
    ...
    var new_num_infected = this._new_random_state(prev_cases, action_r, this.imported_cases_per_step);
    ...
    result_obj["cost_economic"] = this._cost_of_action(action_r);
  ...
  cost(result_obj) {
    return this._cost_of_infections(result_obj["num_infected"]) +
           this._cost_of_action(result_obj["action_r"]) +
  ...
  _new_random_state(num_infected, action_r) {
    ...
    var r = 100000000000000.0;
    var p = lam / (r + lam);
    var new_num_infected = Sampling.NegBinomial(r, 1-p);
    // var dist = new Poisson(lam);
    //var dist = new Uniform(lam, lam);
    //var dist = new Constant(lam);
    return dist;
...
function makeEnv() {
	...
	init_num_infected: 100000, // 100,000 people infected -- we're in the middle of a pandemic!
	R_0: 1.08 ** 10, // infections double every 10 days
	imported_cases_per_step: 1,

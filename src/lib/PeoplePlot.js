    ...
    getFigure() {
        ...
        for (let i1 = 0; i1 < this.model.lines.length; i1++) {
            ...
            for (let i2 = 0; i2 < imdata.data.length / 4; i2++) {
                datacopy[4 * i2] =
                    imdata.data[4 * i2 + 1] +
                    Math.floor((red * (255 - imdata.data[4 * i2 + 1] + imdata.data[4 * i2])) / 255);
                datacopy[4 * i2 + 1] =
                    imdata.data[4 * i2 + 1] +
                    Math.floor((green * (255 - imdata.data[4 * i2 + 1] + imdata.data[4 * i2])) / 255);
                datacopy[4 * i2 + 2] =
                    imdata.data[4 * i2 + 1] +
                    Math.floor((blue * (255 - imdata.data[4 * i2 + 1] + imdata.data[4 * i2])) / 255);
                datacopy[4 * i2 + 3] = 255;
            }
        ...
        for (let i1 = 0; i1 < values.length; i1++) {
            ...
            for (let i2 = 0; i2 < personsCount; i2++) {
                ...
                this.context.putImageData(
                    this.model.lines[i1].imageData,
                    this.positions[i2 + offset][0],
                    this.positions[i2 + offset][1]
                );
            }
            offset += personsCount;
        }

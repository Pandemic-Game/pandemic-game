...
const updateMonthlyIndicators = (turnNumber, monthHistory) => {
    ...
    if (lastMonthlyValues) {
        console.log(
            ...
            totalDeaths,
            totalCosts,
            1.1 * lastMonthlyValues.cases > totalCases,
            lastMonthlyValues.cases < 1.1 * totalCases
        );
    }
    if (lastMonthlyValues && lastMonthlyValues.cases > 1.1 * totalCases) {
        ...
        if (lastMonthlyValues.cases > 1.5 * totalCases) {
            $(`#month-cases-${turnNumber}`).html(
                `${nFormatter(totalCases, 0)}<span class='current-value extreme-negative'></span>`
            );
        } else {
            $(`#month-cases-${turnNumber}`).html(
                `${nFormatter(totalCases, 0)}<span class='current-value negative'></span>`
            );
        }
        ...
        if (1.5 * lastMonthlyValues.cases < totalCases) {
            $(`#month-cases-${turnNumber}`).html(
                `${nFormatter(totalCases, 0)}<span class='current-value extreme-positive'></span>`
            );
        } else {
            $(`#month-cases-${turnNumber}`).html(
                `${nFormatter(totalCases, 0)}<span class='current-value positive'></span>`
            );
        }
    } else {
    ...
    if (lastMonthlyValues && lastMonthlyValues.deaths > 1.1 * totalDeaths) {
        ...
        if (lastMonthlyValues.deaths > 1.5 * totalDeaths) {
            $(`#month-deaths-${turnNumber}`).html(
                `${nFormatter(totalDeaths, 0)}<span class='current-value extreme-negative'></span>`
            );
        } else {
            $(`#month-deaths-${turnNumber}`).html(
                `${nFormatter(totalDeaths, 0)}<span class='current-value negative'></span>`
            );
        }
        ...
        if (1.5 * lastMonthlyValues.deaths < totalDeaths) {
            $(`#month-deaths-${turnNumber}`).html(
                `${nFormatter(totalDeaths, 0)}<span class='current-value extreme-positive'></span>`
            );
        } else {
            $(`#month-deaths-${turnNumber}`).html(
                `${nFormatter(totalDeaths, 0)}<span class='current-value positive'></span>`
            );
        }
    } else {
    ...
    if (lastMonthlyValues && lastMonthlyValues.cost > 1.1 * totalCosts) {
        ...
        if (lastMonthlyValues.cost > 1.5 * totalCosts) {
            $(`#month-cost-${turnNumber}`).html(
                `${nFormatter(totalCosts, 1)}<span class='current-value extreme-negative'></span>`
            );
        } else {
            $(`#month-cost-${turnNumber}`).html(
                `${nFormatter(totalCosts, 1)}<span class='current-value negative'></span>`
            );
        }
        ...
        if (1.5 * lastMonthlyValues.cost < totalCosts) {
            $(`#month-cost-${turnNumber}`).html(
                `${nFormatter(totalCosts, 1)}<span class='current-value extreme-positive'></span>`
            );
        } else {
            $(`#month-cost-${turnNumber}`).html(
                `${nFormatter(totalCosts, 1)}<span class='current-value positive'></span>`
            );
        }
    } else {
